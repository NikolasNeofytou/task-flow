...
class EnhancedChatController extends StateNotifier<Map<String, List<ChatMessage>>> {
  ...
  void sendTextMessage(String channelId, String text) {
    ...
    state = {...state, channelId: [...list, msg]};
    ...
    final updatedMessages = messages.map((msg) {
      if (msg.id == messageId && !msg.isMe) {
        ...
        if (!alreadyViewed) {
          ...
          return msg.copyWith(viewedBy: [...msg.viewedBy, newViewer]);
        }
      }
      return msg;
    }).toList();

    state = {...state, channelId: updatedMessages};
  }
  ...
  void updateMessageViewer(String channelId, String messageId, MessageViewer viewer) {
    ...
    final updatedMessages = messages.map((msg) {
      if (msg.id == messageId) {
        ...
        if (!alreadyViewed) {
          return msg.copyWith(viewedBy: [...msg.viewedBy, viewer]);
        }
      }
      return msg;
    }).toList();
  ...
  void unpinMessage(String channelId, String messageId) {
    ...
    final updatedMessages = messages.map((msg) {
      if (msg.id == messageId) {
        return msg.copyWith(isPinned: false, pinnedBy: null, pinnedAt: null);
      }
      return msg;
    }).toList();
          ...
          : ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return _ChatChannelCard(
                  ...
                  lastMessage: last,
                  isFirst: isFirst,
                  onTap: () => context.go('/chat/${channel.id}', extra: channel.label),
                );
              },
            ),
...
class _EnhancedChatThreadScreenState extends ConsumerState<EnhancedChatThreadScreen> {
  ...
  void initState() {
    ...
    _controller.addListener(() {
      if (!mounted) return;
      setState(() => _currentText = _controller.text);
    });
  }
...
Future<void> _pickImageAndSend() async {
  ...
  if (kIsWeb && f.bytes != null) {
    ...
    pathToSend = 'data:$mime;base64,${base64Encode(f.bytes!)}';
  } else {
    // mobile/desktop
    pathToSend = f.path ?? f.name;
  }

  _sendFile(FileAttachment(
    path: pathToSend,
    name: f.name,
    size: f.size,
...
Future<void> _pickDocumentAndSend() async {
  final res = await FilePicker.platform.pickFiles(
    ...
    allowedExtensions: const [
      'pdf','doc','docx','ppt','pptx','txt','xls','xlsx','csv'
    ],
  );
  if (res == null || res.files.isEmpty) return;
...
class _EnhancedChatMessageState extends ConsumerState<_EnhancedChatMessage> {
  ...
  Widget build(BuildContext context) {
    ...
    final bg = widget.message.isMe ? AppColors.primary : colorScheme.surfaceContainerHighest;
    ...
    showModalBottomSheet(
      ...
      builder: (context) => SafeArea(
        child: Column(
          ...
          children: [
            ListTile(
              leading: Icon(widget.message.isPinned ? Icons.push_pin_outlined : Icons.push_pin),
  ...
  Widget _buildMessageFooter(BuildContext context) {
    final colorScheme = Theme.of(context).colorScheme;
    final textColor = widget.message.isMe ? Colors.white.withOpacity(0.7) : colorScheme.onSurfaceVariant;
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                ...
                leading: CircleAvatar(child: Text(viewer.userName[0].toUpperCase())),
    ...
    switch (widget.message.type) {
      case MessageType.text:
        return Padding(
          ...
          child: Text(
            widget.message.text ?? '',
            style: TextStyle(color: widget.message.isMe ? Colors.white : null),
          ),
        );
          ...
          SizedBox(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return _PinnedMessageCard(message: message, channelId: channelId);
              },
            ),
          ),
        ],
        ...
        children: [
              ...
              children: [
                Row(
                  children: [
                    CircleAvatar(
                      radius: 12,
                      child: Text(message.author[0].toUpperCase(), style: const TextStyle(fontSize: 10)),
                    ),
                    const SizedBox(width: AppSpacing.xs),
                    Expanded(
                      child: Column(
          ...
          Positioned(
            ...
            child: IconButton(
              ...
              onPressed: () {
                ref.read(chatThreadsProvider.notifier).unpinMessage(channelId, message.id);
              },
            ),
          ),
        ],
          ...
          child: Row(
            children: [
              Container(
                ...
                child: Center(
                  child: Text(
                    channel.label.isNotEmpty ? channel.label.substring(0, 1).toUpperCase() : '?',
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: Icon(Icons.chat_bubble_outline, size: 60, color: colorScheme.primary),
            ),
